...
// move base simple goal callback function
void MBSPGoal_Callback(const geometry_msgs::PoseStamped::ConstPtr& goal)
{
    int waypointSize = waypoints->points.size();
    if(waypointSize >= max_waypoint_number){
        ROS_INFO("Max waypoint number reached!");
    } else {
        ROS_INFO("A new goal was added, x: %f, y:%f", goal->pose.position.x, goal->pose.position.y);
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("max_waypoint_number", max_waypoint_number);

  ros::Subscriber subPose = nh.subscribe<nav_msgs::Odometry> ("/state_estimation", 5, poseHandler);
  
  ros::Subscriber sub = nh.subscribe("/move_base_simple/goal", 5, MBSPGoal_Callback);
  ...
  while (status) {
    ...
    if (curTime - waypointTime > 1.0 / frameRate) {
      ...
      waypointSize = waypoints->points.size();
